...
//! in the [`agentsmith-rs-core`][esc] crate. This crate does not perform the checks since it contains the
...
//! `version` field. In this case, use the `agentsmith-rs-core` crate, which bundle the version with
...
//! code that we don't want to hide in a `Debug` impl. See the [`agentsmith-rs-core`][esc] crate, with its
//! higher level types for useful `Debug` impls (and `PartialEq`, `Eq`, `Hash`).
//!
#![doc = concat!("[esc]: https://docs.rs/agentsmith-rs-core/", env!("CARGO_PKG_VERSION"), "/agentsmith-rs-core")]
